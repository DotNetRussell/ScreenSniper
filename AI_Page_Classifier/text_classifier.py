...
class TextClassifier:
    ...
    def load(self, automation=False, retrain=True, model_exists=True):
        if os.path.exists(self.model_file_path):
            checkpoint = torch.load(self.model_file_path, map_location=self.device)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Text classifier using BERT for predicting interesting pages.",
    ...
    try:
        ...
        if args.retrain or (not model_exists and not args.automation):
            if args.automation:
                ...
                sys.exit(1)
            if args.verbose:
                print(f"Fine-tuning BERT with {args.threads} thread{'s' if args.threads > 1 else ''} {'on GPU' if classifier.device.type == 'cuda' else 'on CPU'}...")
            elapsed_time = classifier.train(args.threads)
            if args.verbose:
                print(f"Fine-tuning took {elapsed_time:.2f} seconds")
            classifier.save(classifier.model_file_path)
            if args.verbose:
